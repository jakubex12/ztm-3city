...
async def async_setup_entry(hass, config_entry, async_add_entities):
    ...
    # Dodawanie encji - TYLKO JEDNA ENCJA
    async_add_entities([
        ZTMDepartureSensor(coordinator, stop_name, stop_id),
    ])
...
class ZTMDepartureSensor(CoordinatorEntity, SensorEntity):
    ...
    def __init__(self, coordinator, stop_name, stop_id):
        super().__init__(coordinator) # To kluczowa zmiana - łączy sensor z danymi
        self._stop_name = stop_name
        self._stop_id = stop_id
        
        # Generowanie ID encji - BEZ dopisku "_wszystkie"
        self._attr_unique_id = f"{stop_id}_departures"
        self._attr_name = f"Autobusy {stop_name}"
